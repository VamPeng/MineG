...
- `THEME.md`：以 MineG 蓝色 Logo 为来源的 App 主题色、状态色和使用规则；作为颜色 Token 的唯一基线。
...
备份主页面统一展示设备本地媒体，不展示独立顶部栏。页面自上而下由同步状态和两列相册卡片列表组成；刷新与设置使用图标按钮，放置在同步状态卡右上角。每张相册卡片使用一张缩略图作为封面，相册名称以半透明圆角标签叠放在封面左下角。进入单相册后，媒体使用三列网格排列。
