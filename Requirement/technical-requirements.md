...
- 文档状态：当前实施基线
- 核定日期：2026-07-26
- 最近修订：2026-08-02（基线化阶段 04 持久队列、2×2 并发、网络 Effect 与账号作用域去重）
- 产品基线：[MineG 产品需求 v2.1](./product-requirements.md)
- 功能基线：[MineG 功能需求 v1.2](./functional-requirements.md)
- 移动端契约：[MineG 三端一致性契约 v1.2](../Mobile/three-platform-consistency-contract.md)
...
| `media` | 所有者、拍摄时间、类型、状态、账号作用域内容摘要 |
...
- 客户端计算规范化资源摘要，服务端的去重查询、唯一约束和结果复用必须同时绑定 `owner_id`；不同账号之间禁止比较或复用去重结果。由于 ECS 仍需保存资源 SHA-256 做完整性校验，本方案不把去重描述为对高权限服务端的密码学隔离。
- 本地先按平台 ID、修改版本和下载回执去重，服务端唯一约束作为最终保护。
- 同账号跨设备相同资源命中后，后端返回已有媒体引用；客户端只补齐必要相册关系，不重新上传对象。
- 不同账号禁止执行去重比较或复用结果，始终保留独立媒体记录和对象资源。

### 10.3 重试与并发

- 默认同时推进 2 条媒体、每条媒体同时上传最多 2 个分片，全局最多 4 个对象传输；根据内存、网络和平台后台约束动态下调。
...
| F-06 扫描与本地相册 | 接收相册关系和媒体元数据 | 无 | 平台扫描、索引、增量、缩略图、两列相册卡片、单相册三列网格 | M2～M4 |
| F-07 传输与完整性 | 对象授权、长度/SHA-256 核对、审计 | 无媒体能力 | HTTPS 传输、完整性校验、临时文件清理 | B3～B4 / M3～M5 |
| F-08 上传队列与去重 | 上传会话、STS、分片核对、账号唯一约束 | 无 | 状态机、分块、重试、内容摘要、恢复 | B3 / M3～M4 |
...
- 统一权限页、完整授权检测、默认关闭的自动备份设置、相册分页、两列相册卡片和单相册三列媒体网格。
- 扫描状态先使用本地真实索引，不接假上传进度。
- 冻结 F-04～F-06 的 Port 方法、权限状态、相册模型和 UI 操作契约。

### M3：Android 单媒体备份

- 资源清单、长度/SHA-256、账号作用域去重、HTTPS 私有 OSS 直传、服务端完成确认；不得生成 Media Key 或密文副本。
...
| T-013 | 去重 | 内容/资源清单摘要与所有去重约束绑定账号；禁止跨账号复用结果 |
