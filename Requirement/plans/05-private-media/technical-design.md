...
| `VIEW` | `DETAIL` | 照片/GIF/静态动态媒体的 `ORIGINAL`；视频继续选择预览、缩略图或封面 |
...
- 缩略图继续使用 `VIEW + THUMBNAIL`，不增加 Item 宽高、设备密度或尺寸档位参数；一台设备内同一账号的一条 `media_id` 只保存一张缩略图。
- 逻辑缓存键固定为 `account_id + media_id`，落盘文件名使用带版本命名空间的 SHA-256，不使用、持久化或记录会变化的 OSS 签名地址。
- L1 使用 Coil 解码内存 LRU，硬上限为 `min(设备最大堆内存的 10%, 32 MiB)`；L2 使用应用私有、不可备份的磁盘目录，超过 `256 MiB` 后按最近访问时间清理至 `192 MiB`。
- 只有完成既有长度/SHA-256校验或 OSS 动态缩略图安全校验的文件才允许原子写入 L2；磁盘记录保存本地输出长度和 SHA-256，App 重启后首次命中重新核对，失败时删除并按普通未命中重新获取。
- 缓存命中只在当前已恢复的同账号会话内使用，不续签也不保存对象授权；未命中仍完整执行现有访问授权和受控下载链路。
- 手动重试、媒体移入回收站和退出登录清理对应缓存；普通列表刷新不清空有效缩略图。App 启动时清除未完成临时文件，缓存容量回收不得删除正在展示的文件。
- 缓存是可重建的图片字节与解码结果，不作为领域列表、授权状态或媒体完成状态的真实来源。
...
服务端 DTO 不携带设备标识。Core 生成列表和详情快照时，使用同账号的
`backup_tasks.server_media_id` 与 `download_receipts.cloud_media_id` 联查当前有效本地索引；
只有映射条目仍处于活动扫描代次且 `availability=AVAILABLE` 时，才追加可空的
`localPlatformAssetRef` 和 `localSourceUri`。上传映射还必须保持 `content_version` 一致，
下载回执必须保持 `content_revision` 一致；失效映射按无本地资源处理。
...
- 私人网格始终使用云端缩略图/封面，不因存在本地映射或私有原图副本而解码原图；进入本人照片/GIF/静态动态媒体详情后，Android 依次尝试：实际可打开的 MediaStore 映射、App 私有原图文件、`VIEW + DETAIL` 云端原图下载；视频仍使用封面/预览播放链路，不把完整视频交给图片解码器；
- Core 的 `AVAILABLE` 只代表最近扫描快照。Android 采用映射前必须实际打开 MediaStore 条目；失败即视为映射失效。云端原图完成长度/SHA-256 校验后，原子写入 `noBackupFilesDir/mineg-originals-v1/<account-scope>/<mediaId>`，数据文件名必须严格等于 `mediaId`；每次复用都重新对照详情资源清单校验长度和 SHA-256，失败立即删除并重新下载；
- 私有原图目录按账号隔离，退出账号和媒体进入回收站时删除对应数据；不得写入签名 URL、对象键或授权头；
