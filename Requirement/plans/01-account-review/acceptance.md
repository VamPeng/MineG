...
> 历史证据说明：以下结果如实记录 2026-07-26 当时的旧 key-grant 流程，不代表当前产品行为。批次 E1 已改为无 key bundle 注册、管理员审核后直接 `APPROVED`；新验收不得复用本文的 `PENDING:key-grant` 结论。

> 2026-07-30 架构复核附注：注册—审核—登录产品闭环、安全边界和测试证据仍有效；账号 API/DTO、Session/Token 编排、审核状态与资料缓存当前仍位于 Android Kotlin，未满足技术需求 v1.2 的 C++ 数据主权门禁，必须按[迁移文档](../../../Mobile/docs/android-data-layer-migration.md)整改后才能作为三端共享数据层验收。
