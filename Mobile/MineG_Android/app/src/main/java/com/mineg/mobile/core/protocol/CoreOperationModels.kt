/**
 * Immutable Kotlin representation of the frozen Core/platform effect protocol.
 *
 * Parsing and serialization validate envelope identity before values reach domain gateways.
 */
...
enum class PlatformEffectType(val wireName: String) {
  ...
  companion object {
    /** Resolves the frozen wire discriminator and rejects unknown effect families. */
...
data class PlatformEffectError(
  ...
  /** Serializes a platform failure for inclusion in an effect result. */
...
data class PlatformEffectResult(
  ...
  /** Serializes the effect result envelope consumed by native Core. */
...
data class CoreOperationStep(
  ...
  companion object {
    /** Parses and validates one operation-step envelope returned by Core. */
    fun parse(json: String): CoreOperationStep = parseObject(JSONObject(json))

    /** Parses durable operation steps recovered after Android process restart. */
    ...
    /** Enforces envelope identity and maps its optional effect/result/error branches. */
